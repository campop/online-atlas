...
projectlight.createCarousel = function(){
	...
	carousel.init = function(){
		this.carouselContainer = $("#campl-carousel");
...
//DPC: Project light fix to external URLs doesn't work with JQuery 1.4.4
//DPC: as $(this).attr("title") throws exceptions if undefined.
//DPC: Here is lrowe's original fixed version, which actually works!
projectlight.markExternalLinks = function(){
	var external_links;
	external_links = $('#content a').not(".campl-carousel a").filter(function(){
	    return(this.hostname &&
               this.hostname !== location.hostname &&
               !this.hostname.match(/\.cam\.ac\.uk$/gi));
		});
	external_links.addClass("campl-external").attr("target", "_blank").attr("title", function(index, attr){
		return ((attr) ? (attr+" ") : "") + "(Link to an external website)";
	})
}
...
// DPC: Can only work out correct height of all elements when page load is
// DPC: complete. jQuery load handler better than window.onload.
